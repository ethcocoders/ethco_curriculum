document.addEventListener('DOMContentLoaded', function() {
    const contentItemList = document.getElementById('content-item-list'); // Target the new div container
    const saveOrderBtn = document.getElementById('saveOrderBtn');
    const parentId = saveOrderBtn.dataset.parentId; // Get the parent submodule ID
    let initialOrder = [];

    if (contentItemList) {
        // Capture initial order
        Array.from(contentItemList.children).forEach(col => {
            const card = col.querySelector('.content-item-card');
            if (card) {
        ...
        if (typeof Sortable !== 'undefined') {
            const sortable = new Sortable(contentItemList, {
                handle: '.handle', // Drag handle within the card
                animation: 150,
                ghostClass: 'sortable-ghost', // Class for the ghost element (defined in modern.css)
                onEnd: function (evt) {
                    const newOrder = Array.from(contentItemList.children).map(col => col.dataset.id);
        ...
        saveOrderBtn.addEventListener('click', function() {
            ...
            .then(data => {
                if (data.status === 'success') {
                    alert('Content item order updated successfully!');
                    ...
                    alert('Error updating content item order: ' + data.message);
                }
            })
            .catch(error => {
